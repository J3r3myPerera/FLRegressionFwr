...
from module import Net, load_centralized_dataset, test, NUM_CLIENTS, BATCH_SIZE, FRACTION_FIT, LOCAL_EPOCHS, LEARNING_RATE, DEVICE
...
class FederatedSimulator:
    ...
    def run(self, num_rounds: int) -> dict:
        ...
        for round_num in range(1, num_rounds + 1):
            ...
            loss, accuracy = self.evaluate_global(global_state)
            ...
            print(f"    Accuracy = {accuracy:.4f}, Loss = {loss:.4f}, Avg μ = {avg_mu:.4f}")

        print(f"\n  Final Accuracy: {metrics['accuracies'][-1]:.4f}")
        return metrics
